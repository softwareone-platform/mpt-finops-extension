...
def test_generate_refunds_success(
    ...
    assert response[0].start_date == datetime(2025, 6, 1, 0, 0, tzinfo=timezone.utc)
    assert response[0].end_date == datetime(2025, 6, 15, 23, 59, 59, tzinfo=timezone.utc)
...
def test_generate_refunds_success_trial_and_entitlements(
    ...
    assert response[1].start_date == datetime(
        2025, 6, 16, hour=0, minute=0, second=0, microsecond=0, tzinfo=timezone.utc
    )
    assert response[1].end_date == datetime(
        2025, 6, 30, hour=23, minute=59, second=59, microsecond=0, tzinfo=timezone.utc
    )
...
def test_generate_refunds_no_trial_days_active_entitlement(
    ...
    assert response[0].start_date == datetime(
        2025, 6, 1, hour=0, minute=0, second=0, microsecond=0, tzinfo=timezone.utc
    )
    assert response[0].end_date == datetime(
        2025, 6, 30, hour=23, minute=59, second=59, microsecond=0, tzinfo=timezone.utc
    )
...
def test_generate_refunds_no_entitlement_end_date(
    ...
    assert response[1].description == "Refund due to active entitlement FENT-2502-5308-4600"
    assert response[1].start_date == datetime(
        2025, 6, 16, hour=0, minute=0, second=0, tzinfo=timezone.utc
    )
    assert response[1].end_date == datetime(
        2025, 6, 30, hour=23, minute=59, second=59, tzinfo=timezone.utc
    )
...
def test_get_trial_days(agreement_data_with_trial):
    trial_start_to_match = datetime(
        2025, 6, 1, hour=0, minute=0, second=0, microsecond=0, tzinfo=timezone.utc
    )
    trial_end_to_match = datetime(
        2025, 6, 15, hour=23, minute=59, second=59, microsecond=0, tzinfo=timezone.utc
    )
    data = agreement_data_with_trial()
...
def test_get_trial_days_partial_overlap(billing_process_instance):
    trial_start = datetime(2025, 5, 25, hour=0, minute=0, second=0, tzinfo=timezone.utc)
    trial_end = datetime(2025, 6, 5, hour=23, minute=59, second=59, tzinfo=timezone.utc)
    billing_start = datetime(2025, 6, 1, 0, 0, tzinfo=timezone.utc)

    trial_info = billing_process_instance.get_trial_info(trial_start, trial_end)

    assert trial_info.refund_from == billing_start
    assert trial_info.refund_to == datetime(2025, 6, 5, 23, 59, 59, tzinfo=timezone.utc)
    assert trial_info.trial_days == set(range(1, 6))


def test_get_trial_days_full_month(billing_process_instance):
    trial_start = datetime(2025, 6, 1, hour=0, minute=0, second=0, tzinfo=timezone.utc)
    trial_end = datetime(2025, 6, 30, hour=23, minute=59, second=59, tzinfo=timezone.utc)
    billing_start = datetime(2025, 6, 1, 0, 0, tzinfo=timezone.utc)

    trial_info = billing_process_instance.get_trial_info(trial_start, trial_end)

    assert trial_info.refund_from == billing_start
    assert trial_info.refund_to == datetime(2025, 6, 30, 23, 59, 59, tzinfo=timezone.utc)
