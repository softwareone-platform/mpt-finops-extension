...
def send_mpt_notification(client: MPTClient, order_context: type[OrderContext]) -> None:
    ...
    template_context = {
        ...
        "activation_template": md2html(
            get_rendered_template(client, order_context.order_id)
        ),
    ...
    if order_context.order["status"] == "Querying":
        subject = (
            f"This order need your attention {order_context.order_id} "
            f"for {buyer_name}"
        )
    mpt_notify(
        client,
        order_context.order["agreement"]["licensee"]["account"]["id"],
